...
class DUMMY:
    ...
    def __init__(self):
        self.ue = pd.read_csv('C:/Users/douni/anaconda3/envs/xappr/ad_dummy/ad/valid.csv')
        ...
        self.label = self.test[['Anomaly']]
        self.data = None

    def read_data(self, meas='ueMeasReport', limit=100000):
        if meas == 'train':
            self.data = self.train.head(limit).drop('Anomaly', axis=1)
        elif meas == 'valid':
            self.data = self.train.head(limit)
        else:  #'liveUE'
            self.data = self.test.head(limit).drop('Anomaly', axis=1)


    def write_anomaly(self, df, meas_name='AD'):
        if os.path.isfile('ad_results.csv'):
            df.to_csv('ad_results.csv', header=None, mode="a")
        else:
            df.to_csv('ad_results.csv')
