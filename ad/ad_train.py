...
def train(thread=True):
    ...
    for of in np.arange(0.01, 0.4, 0.01):
        scores.append(mod.isoforest(outliers_fraction=of)) #prediction score
    opt_f1 = scores.index(max(scores)) + 1
    #IF model with optimum contamination
    opt_score = mod.isoforest(outliers_fraction=opt_f1*0.01, push_model=True)
    print("Optimum value of contamination : {}".format(opt_f1*0.01))
    print("With a f1_score of : {}".format(opt_score))
    print('Training Ends : ')


def test(self, model):
    pred = model.predict(self.X)
    if -1 in pred: #if anomaly
        pred = [1 if p == -1 else 0 for p in pred]
    return f1_score(self.actual, pred) # if 1 -> same values
