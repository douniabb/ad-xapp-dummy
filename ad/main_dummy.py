...
from ad_model.ad_model_AE import ad_predict, CAUSE
from ad_train_AE import train
...
def predict():
    ...
    global pos
    pos = pos % len(ue_data)  # iterate through entire list one by one in cycle manner and will be updated when live feed will be coming through KPIMON to influxDB
    sample = ue_data[pos]
    pos = pos+1
    ue_df = pd.DataFrame([sample], columns=db.data.columns)
    val = predict_anomaly(ue_df)
...
def msg_to_ts( val):
    ...
    print("[INFO] Sending Anomalous UE to TS :", val, "\n")
...
def connectdb(thread=True):
    # Create a connection to InfluxDB if thread=False, otherwise it will create a dummy data instance
    global db
    global cp
    global ue_data
    if thread:
...
def accuracy():
    df = pd.read_csv('ad_results.csv')
    pred = df['Anomaly']
    
    print("Accuracy f1 score : {}".format(f1_score(db.label, pred)))
